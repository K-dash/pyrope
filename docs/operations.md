...
- `map_py(func: Callable[[T], U])`
  - Input: object
  - Output: object
  - Behavior: calls the Python function with the current value. Exceptions are converted to `Error` with code `py_exception` and traceback stored in `metadata["py_traceback"]`.
